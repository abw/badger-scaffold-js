...
import { absRelPath, ifLog, maybeDefault } from './Utils.js'
...
export async function configure({ root, help, version }) {
  ...
  const scaffoldConfig = await scaffoldWs.config(maybeDefault(VARS), { })
  ...
  const templateConfig = await templateWs.config(maybeDefault(VARS), { })
  ...
  const files = await templateWs.config(maybeDefault(FILES), { })
